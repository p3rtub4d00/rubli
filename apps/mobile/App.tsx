...
export default function App() {
  ...
  async function ensureConversation(demandId: string, providerId: string) {
    const customerId = demands.find((item) => item.id === demandId)?.requesterId;
    if (!customerId) return null;
    const existing = conversations.find((item) => item.demandId === demandId && item.customerId === customerId && item.providerId === providerId);
    if (existing) return existing;
    const now = new Date().toISOString();
    const conversation: Conversation = { id: newId('conv'), demandId, customerId, providerId, createdAt: now, updatedAt: now };
  ...
  async function openProposalChat(proposal: Proposal) {
    const conversation = await ensureConversation(proposal.demandId, proposal.providerId);
  ...
  async function sendMessage(text: string) {
    ...
    const nextConversations = conversations.map((item) => item.id === activeConversation.id ? { ...item, updatedAt: message.createdAt, lastMessageAt: message.createdAt } : item);
  ...
  return <SafeAreaView style={styles.safeLight}><StatusBar style="dark" /><View style={styles.header}><View><Text style={styles.brand}>Rubli</Text><Text style={styles.headerSubtitle}>Olá, {user.name}</Text></View><TouchableOpacity onPress={() => setScreen('profile')} style={styles.avatar}><Text style={styles.avatarText}>{user.name[0]?.toUpperCase()}</Text></TouchableOpacity></View>
    ...
    {screen === 'home' && user.role === 'provider' && <ProviderHome user={user} feed={providerFeed} proposals={proposals} providerRadius={providerRadius} providerLatitude={providerLatitude} urgentOnly={urgentOnly} selectedTypeFilter={selectedTypeFilter} proposalAmounts={proposalAmounts} proposalMessages={proposalMessages} setUrgentOnly={setUrgentOnly} setSelectedTypeFilter={setSelectedTypeFilter} setProposalAmounts={setProposalAmounts} setProposalMessages={setProposalMessages} onLocate={captureProviderLocation} onRadius={updateProviderSettings} onProposal={submitProposal} onViewNegotiation={openProposalChat} />}
...
async function acceptProposal(proposal: Proposal) { /* callback placeholder kept for the local-first home flow */ }
