...
export function ChatScreen({ conversation, currentUserId, otherUserName = 'Usuário', providerProfile = null, providerRatings = [], onBack, onAcceptProposal, onConfirmAgreement, onCounterProposal, onServiceAction }: ChatScreenProps) {
  ...
  async function reload() {
    ...
    const providerId = demandItem?.acceptedProviderId ?? conversation.providerId;
    const fallbackHistory = history.length > 0
      ? history
      : proposalItems.filter((item) => item.demandId === conversation.demandId && item.providerId === providerId).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    setProposalHistory(fallbackHistory);
    setProposal(fallbackHistory[fallbackHistory.length - 1] ?? null);
