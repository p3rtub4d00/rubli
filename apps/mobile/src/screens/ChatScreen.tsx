...
export function ChatScreen({ conversation, currentUserId, otherUserName = 'Usuário', onBack, onAcceptProposal, onConfirmAgreement }: ChatScreenProps) {
  ...
  useEffect(() => {
    Promise.all([getMessages(), getProposals()]).then(([items, proposals]) => {
      ...
      const linked = proposals.filter((item) => item.demandId === conversation.demandId).sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        ...
        <View style={styles.headerText}><Text style={styles.title}>{otherUserName}</Text><Text style={styles.subtitle}>{bothConfirmed ? 'Serviço contratado' : 'Negociação pelo Rubli'}</Text></View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.messages} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.notice}><Text style={styles.noticeTitle}>Negociação vinculada à demanda</Text><Text style={styles.noticeText}>Combine valores, horário e detalhes aqui. A confirmação fica registrada para os dois lados.</Text></View>

        {currentConversationMessages.length === 0 ? <Text style={styles.empty}>Nenhuma mensagem ainda. Comece a conversa.</Text> : currentConversationMessages.map((message) => {
          const mine = message.senderId === currentUserId;
          return <View key={message.id} style={[styles.bubble, mine ? styles.mine : styles.theirs]}><Text style={mine ? styles.mineText : styles.theirsText}>{message.text}</Text><Text style={mine ? styles.mineTime : styles.theirsTime}>{new Date(message.createdAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</Text></View>;
        })}
      </ScrollView>
