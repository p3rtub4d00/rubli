...
import { apiUpdateUserProfile } from '../api/client';
...
export function MyProfileScreen({ user, onSaved, onClose }: Props) {
  ...
  async function save() {
    ...
    const nextUser = await apiUpdateUserProfile(user.id, { name: name.trim(), bio: bio.trim() || undefined, city: city.trim() || undefined, serviceRadiusKm: user.role === 'provider' ? radius : undefined, serviceCategories: user.role === 'provider' ? categories : undefined, avatarUri, profilePhotos: photos, professionalTitle: user.role === 'provider' ? professionalTitle.trim() || undefined : undefined, businessAddress: user.role === 'provider' && user.taxDocumentType === 'cnpj' ? businessAddress.trim() || undefined : undefined });
