...
export async function apiSearchPremiumProviders(input: { query?: string; providerType?: ProviderType; page?: number; limit?: number; sort?: 'relevant' | 'distance' | 'rating' | 'completed' | 'available'; available?: boolean; verified?: boolean; minRating?: number; radius?: number; latitude?: number; longitude?: number } = {}) { const query = new URLSearchParams(); Object.entries(input).forEach(([key, value]) => { if (value !== undefined && value !== '') query.set(key, String(value)); }); return request<PremiumProviderSearchResult>(`/api/v1/providers/search?${query.toString()}`); }
