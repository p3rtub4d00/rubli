...
async function readJson<T>(key: string, fallback: T): Promise<T> { const value = await AsyncStorage.getItem(key); return value ? (JSON.parse(value) as T) : fallback; }
...
export async function saveConversations(conversations: Conversation[]) {
  ...
  try { await Promise.all(conversations.map((conversation) => apiCreateConversation(conversation))); } catch {}
}
