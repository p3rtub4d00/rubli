...
export async function registerProposalRoutes(app) {
    ...
    app.post('/api/v1/proposals', async (request, reply) => {
        ...
        const demand = memoryStore.demands.find((item) => item.id === body.demandId);
        if (!demand) {
    ...
    app.post('/api/v1/proposals/:id/accept', async (request, reply) => {
