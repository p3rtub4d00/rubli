...
export async function registerChatRoutes(app) {
    app.get('/api/v1/conversations', async (request) => {
        ...
        return memoryStore.conversations.filter((conversation) => (!demandId || conversation.demandId === demandId) &&
    ...
    app.post('/api/v1/conversations', async (request, reply) => {
        ...
        const existing = memoryStore.conversations.find((item) => item.demandId === body.demandId && item.customerId === body.customerId && item.providerId === body.providerId);
        if (existing)
            return existing;
        const now = new Date().toISOString();
        const conversation = { ...body, id: id('conv'), createdAt: now, updatedAt: now };
