...
export async function registerNearbyRoutes(app) {
    app.get('/api/v1/demands/nearby', async (request, reply) => {
        const latitude = Number(request.query.latitude);
        const longitude = Number(request.query.longitude);
        const radiusKm = Math.min(Math.max(Number(request.query.radiusKm) || DEFAULT_RADIUS_KM, 1), MAX_RADIUS_KM);
        ...
        const results = memoryStore.demands
            ...
            .map((demand) => ({
            demand,
            distanceKm: distanceKm({ latitude, longitude }, { latitude: demand.latitude, longitude: demand.longitude }),
        }))
